...
def calculate_return_period(events, period, surge_bins): 
   
    # Parameters  
    period = 460*23

    #surge_bins = numpy.arange(0,6)

    surge_frequency, surge_bins = numpy.histogram(events, surge_bins) 
    
    N = surge_frequency.sum() 

    # Complement of the empirical cumulative distribution function
    # Calculate cum sum of surge frequencies in reversre order then 
    # return them in reverse order (this returns the exceedance) 
    # Divide by N to get the probabilities of each exceedance level  
    edcf_c = numpy.cumsum(surge_frequency[::-1])[::-1] * 1.0/N

    empirical_return_period = period * 1.0/(N * edcf_c) 

    return surge_frequency, surge_bins, empirical_return_period  
    
    

def plot_return_perod(path, plot_output, job_name, agg_data, format='png'):  
